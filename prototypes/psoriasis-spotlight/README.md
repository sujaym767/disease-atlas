...
**Open [`index.html`](index.html)** (self-contained, offline). Drag to pan, scroll to zoom, click any entity. Previews: [poster](preview_poster.png) · [central schematic](preview_schematic.png) · [cell ER popup](preview_bio_cell.png) · [2-hop](preview_2hop.png) · [network explorer](preview_network.png) · [path trace](preview_path.png).
...
## Build
`build_deep.py` (authors the graph from the live roster + cited facts, incl. the 11 **Biology** cascade nodes and their bridge edges → `graph.json`) → `build.py` (injects into `template.html` → `index.html`). `verify.py` headless-checks it (no JS errors; cell click; 2-hop; network expand; path trace) and regenerates the previews.
```bash
